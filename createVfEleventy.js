    ...
    if (appName) {
      try {
        const spinner = new ora({
          prefixText: '🌍 ',
        	text: 'Fetching github.com/visual-framework/vf-eleventy/archive/v2.0.0-alpha.2.zip',
          // indent: 2,
        	// spinner: 'pong'
        });


        download('https://github.com/visual-framework/vf-eleventy/archive/v2.0.0-alpha.2.zip').then(data => {
          fs.writeFileSync('vf-eleventy.zip', data);
          resolve(true)
          spinner.text = 'Fetched github.com/visual-framework/vf-eleventy/archive/v2.0.0-alpha.2.zip';
          spinner.succeed();
        });
...
const moveFiles = () => {
  return new Promise((resolve) => {
    ...
    mv('temp/vf-eleventy-2.0.0-alpha.2', appName, {mkdirp: true}, function(err) {
